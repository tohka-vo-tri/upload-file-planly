 ...
 */
ipcMain.handle('connect-planly', async (event, token: string, teamId: string) => {
  try {
    // Validate inputs
    if (!token || !teamId) {
...
/**
 * Step 4: Upload video to Planly
 */
/**
 * Upload hàng loạt video và tự động phân bổ cho nhiều channels
    ...
    if (bulkUploadState.currentPhase === 'upload' && remainingSchedules.length > 0) {
      ...
      for (let i = 0; i < remainingSchedules.length; i += CONCURRENT_UPLOADS) {
        ...
        if (i + CONCURRENT_UPLOADS < remainingSchedules.length) {
          console.log('[BULK] Waiting 2s before next batch...');
